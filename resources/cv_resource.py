    ...
    def post(self):
        ...
        for experience in work_experience:
            if 'description' not in experience:
                prompt = f"Describe the responsibilities and achievements of a {experience['job_title']} at {experience['company']} using {', '.join(experience['skills'])}."
        ...
        byte_obj = generate_pdf(cv_data=cv, filename="cv.pdf")
        # Return the PDF content as a response
        response = make_response(byte_obj)
        response.headers['Content-Type'] = 'application/pdf'
        response.headers['Content-Disposition'] = 'attachment; filename=my_cv.pdf'
        return response
